...
namespace EVChargingSystem.Api.Services
{
    public class BookingService : IBookingService
    {
        ...
        public async Task<BookingDto> StartCharging(Guid bookingId, string startedBy)
        {
            var booking = await _context.Bookings.Find(b => b.Id == bookingId).FirstOrDefaultAsync();
            if (booking == null || !booking.IsActive)
                throw new Exception("Booking not found or inactive");
            ...
            var update = Builders<Booking>.Update
                .Set(b => b.Status, BookingStatus.Active)
                .Set(b => b.ApprovedBy, startedBy)
                .Set(b => b.ApprovedAt, DateTime.UtcNow);

            await _context.Bookings.UpdateOneAsync(b => b.Id == bookingId, update);

            booking.Status = BookingStatus.Active;
            booking.ApprovedBy = startedBy;
            booking.ApprovedAt = DateTime.UtcNow;

            return MapToDto(booking);
        }

        public async Task<BookingDto> CompleteCharging(Guid bookingId, string completedBy)
        {
            var booking = await _context.Bookings.Find(b => b.Id == bookingId).FirstOrDefaultAsync();
            if (booking == null || !booking.IsActive)
                throw new Exception("Booking not found or inactive");
        ...
        public async Task<IEnumerable<BookingDto>> GetActiveBookings()
        {
            var bookings = await _context.Bookings
                .Find(b => b.Status == BookingStatus.Active && b.IsActive)
        ...
        public async Task<Guid?> GetBookingIdFromQRCode(string qrCodeData)
        {
            try
            {
                var bookingData = await _qrCodeService.ValidateQRCode(qrCodeData);
        ...
        public async Task<IEnumerable<BookingDto>> GetBookingsByStation(Guid stationId)
        {
            var bookings = await _context.Bookings
                .Find(b => b.ChargingStationId == stationId)
                .SortByDescending(b => b.BookingDate)
        ...
        public async Task<IEnumerable<BookingDto>> GetActiveBookingsByStation(Guid stationId)
        {
            var bookings = await _context.Bookings
                .Find(b => b.ChargingStationId == stationId && 
        ...
        public async Task<IEnumerable<BookingDto>> GetPendingBookingsByStation(Guid stationId)
        {
            var bookings = await _context.Bookings
                .Find(b => b.ChargingStationId == stationId && 
        ...
        public async Task<BookingDto> ChangeBookingStatus(Guid bookingId, BookingStatus newStatus, string changedBy, string? reason = null)
        {
            var booking = await _context.Bookings.Find(b => b.Id == bookingId).FirstOrDefaultAsync();
            if (booking == null || !booking.IsActive)
                throw new Exception("Booking not found or inactive");
            ...
            var updateBuilder = Builders<Booking>.Update.Set(b => b.Status, newStatus);
            ...
            switch (newStatus)
            {
                case BookingStatus.Active:
                    if (booking.Status != BookingStatus.Approved)
                        throw new Exception("Only approved bookings can be set to active");
                    break;

                case BookingStatus.Completed:
                    if (booking.Status != BookingStatus.Active)
                        throw new Exception("Only active bookings can be completed");
                    updateBuilder = updateBuilder.Set(b => b.IsActive, false);
                    break;
                ...
                case BookingStatus.Rejected:
                    if (!string.IsNullOrEmpty(reason))
                        updateBuilder = updateBuilder.Set(b => b.RejectionReason, reason);
                    break;
            }

            // Update timestamp and user info
            updateBuilder = updateBuilder
                .Set(b => b.ApprovedBy, changedBy)
                .Set(b => b.ApprovedAt, DateTime.UtcNow);

            await _context.Bookings.UpdateOneAsync(b => b.Id == bookingId, updateBuilder);

            // Update local object
            booking.Status = newStatus;
            booking.ApprovedBy = changedBy;
            booking.ApprovedAt = DateTime.UtcNow;

            if (newStatus == BookingStatus.Completed || newStatus == BookingStatus.Cancelled)
                booking.IsActive = false;

            if (newStatus == BookingStatus.Rejected && !string.IsNullOrEmpty(reason))
                booking.RejectionReason = reason;
        ...
        private static void ValidateStatusTransition(BookingStatus currentStatus, BookingStatus newStatus)
        {
            ...
            {
                { BookingStatus.Pending, new[] { BookingStatus.Approved, BookingStatus.Rejected, BookingStatus.Cancelled } },
                { BookingStatus.Approved, new[] { BookingStatus.Active, BookingStatus.Cancelled } },
                { BookingStatus.Active, new[] { BookingStatus.Completed } },
                { BookingStatus.Rejected, new BookingStatus[0] },
                { BookingStatus.Completed, new BookingStatus[0] },
                { BookingStatus.Cancelled, new BookingStatus[0] }
            };
