...
const transformBookingFromBackend = (backendBooking: any): Booking => {
  ...
  return {
    ...
    chargingStationId:
      backendBooking.chargingStationId || backendBooking.stationId,
    reservationDateTime:
      backendBooking.reservationDateTime || backendBooking.dateTime,
    status:
...
export const bookingApi = {
  ...
  getAll: async (): Promise<Booking[]> => {
    try {
      ...
      if (
        ...
        console.log(
          `📋 Extracted ${bookingsData.length} bookings from response object`
        );
      }
      ...
      console.log(
        "⚠️ Main endpoint failed, trying status endpoints as fallback..."
      );
      try {
  ...
  getOperatorStationBookings: async (): Promise<Booking[]> => {
    try {
      console.log(
        "📡 Fetching operator station bookings - trying base endpoint first..."
      );
      ...
      try {
        ...
        const response = await apiClient.get<any[]>(
          "/api/Bookings/operator/my-station-bookings"
        );
        ...
        if (Array.isArray(bookingsData) && bookingsData.length > 0) {
          ...
          console.log(
            `✅ Successfully fetched ${transformedBookings.length} bookings from base operator endpoint`
          );
          return transformedBookings;
        }
      } catch (baseError: any) {
      ...
      const bookingPromises = [
        ...
        apiClient
          .get<any[]>("/api/Bookings/operator/my-station-bookings/pending")
          .then((res) => {
        ...
        // Get active bookings
        apiClient
          .get<any[]>("/api/Bookings/operator/my-station-bookings/active")
          .then((res) => {
      ...
      const uniqueBookings = Array.from(
        new Map(
          allBookingsData.map((booking) => [booking.id || booking._id, booking])
        ).values()
      );

      console.log(
        `📋 Combined ${uniqueBookings.length} unique bookings from status endpoints`
      );

      if (!Array.isArray(uniqueBookings)) {
        console.warn(
          "❌ Operator bookings data is not an array:",
          uniqueBookings
        );
        return [];
      }

      const transformedBookings = uniqueBookings.map(
        transformBookingFromBackend
      );
      console.log(
    ...
    try {
      ...
      const bookingPromises = [
        ...
        apiClient
          ...
          .catch((err) => {
            console.log(
              `⚠️ Could not fetch pending bookings:`,
              err.response?.status
            );
            return [];
          }),
        ...
        apiClient
          ...
          .catch((err) => {
            console.log(
              `⚠️ Could not fetch approved bookings:`,
              err.response?.status
            );
            return [];
          }),
        ...
        apiClient
          ...
          .catch((err) => {
            console.log(
              `⚠️ Could not fetch rejected bookings:`,
              err.response?.status
            );
            return [];
          }),
        ...
        apiClient
          .get<any>("/api/Bookings/completed")
          .then((res) => {
            const bookings = res.data?.bookings || res.data || [];
          ...
          .catch((err) => {
            console.log(
              `⚠️ Could not fetch completed bookings:`,
              err.response?.status
            );
            return [];
          }),
        ...
        apiClient
          .get<any>("/api/Bookings/cancelled")
          .then((res) => {
            const bookings = res.data?.bookings || res.data || [];
          ...
          .catch((err) => {
            console.log(
              `⚠️ Could not fetch cancelled bookings:`,
              err.response?.status
            );
            return [];
          }),
  ...
  validateQR: async (
    qrCodeData: string
  ): Promise<{ valid: boolean; booking?: Booking; message: string }> => {
